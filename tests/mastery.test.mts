/**
 * The two spec §7 rules that were written down and never built. **Mastery** — "a unit is complete
 * at ≥80% of its words learned".
...
async function pctFor(unitId: string, ord: number): Promise<number> {
  ...
  const html = await res.text();
  const m = html.match(
    new RegExp(`Unit ${ord} · [^"]*?— (\\d+)% der Wörter sitzen`),
  );
  return m ? Number(m[1]) : -1;
}
...
const unit = db
  .prepare(
    "SELECT id, ord, word_ids_json, grammar_id FROM unit WHERE level='A1.1' ORDER BY ord LIMIT 1",
  )
  .get() as {
  id: string;
  ord: number;
...
ok(
  fin.unitDone === true,
  "the unit completes on coverage",
  `wordsLeft=${fin.wordsLeft}`,
);

ok(
  (await sitzen()) === 0,
  "Der Weg counts nothing as sticking yet",
  `${await sitzen()} sitzen`,
);
ok(
  (await pctFor(unit.id, unit.ord)) === 0,
  "0% of its words are learned",
...
ok(
  after.unit?.id !== unit.id,
  "the next unit is offered immediately",
  after.unit?.id,
);
ok(
  after.unit?.ord === unit.ord + 1,
  "and it is the next one in order",
  after.unit?.ord,
);
...
ok(
  (await sitzen()) === 1,
  "at 80% Der Weg counts it as sticking",
  `${await sitzen()} sitzen`,
);
ok(
  (await pctFor(unit.id, unit.ord)) >= 80,
  "and reports the real share",
  `${await pctFor(unit.id, unit.ord)}%`,
);
...
ok(
  (await sitzen()) === 0,
  "lapsing drops it back below the line",
  `${await sitzen()} sitzen`,
);
const stillDone = await get(`/api/session?user=${U}`);
ok(
  stillDone.unit?.id !== unit.id,
  "but the unit is still finished — coverage does not regress",
...
const [second, third, far] = db4
  .prepare(
    "SELECT id, ord, prereq_json FROM unit WHERE level='A1.1' AND ord IN (2,3,9) ORDER BY ord",
  )
...
ok(
  gated.unit?.id === third.id,
  "the gated unit is skipped and the available one is taken",
...
db5
  ...
  .run(third.prereq_json, third.id);
db5.close();

const deadlocked = await get(`/api/session?user=${U}`);
ok(
  deadlocked.unit?.id === second.id,
  "with everything blocked, prerequisites are ignored rather than the learner stranded",
  deadlocked.unit?.id,
);
ok(
  !deadlocked.unit?.id?.startsWith("b1-2"),
  "and nobody is flung to the end of the course",
  deadlocked.unit?.id,
);
...
ok(
  typo.unit?.id === second.id,
  "an unknown prerequisite is ignored",
  typo.unit?.id,
);
