/**
 * Grammar rules are scheduled, not just displayed once. A rule taught in unit 3 and never seen
 * again is a rule you do not have.
...
const db0 = open();
const gid = (
  db0.prepare("SELECT id FROM grammar ORDER BY ord LIMIT 1").get() as {
    id: string;
  }
).id;
db0.close();
...
function card() {
  ...
  const r = db
    .prepare(
      "SELECT id, reps, due, state FROM card WHERE user_id = ? AND ref_type = 'grammar' AND ref_id = ?",
    )
    .get(U, gid) as
    { id: number; reps: number; due: string; state: number } | undefined;
  db.close();
  return r;
}
...
ok(
  new Date(c!.due.replace(" ", "T") + "Z") > new Date(),
...
ok(
  !s.blocks.some((b: any) => b.kind === "grammar-review"),
...
ok(
  !!gr,
  "the grammar review appears",
  s2.blocks.map((b: any) => b.kind).join(" "),
);
...
ok(
  gc.drills.length > 0,
  "it carries drills",
  `${gc.drills.length} drills · ${gc.title}`,
);
...
ok(
  ...
  "grading advances the card",
  `${beforeReps} -> ${card()!.reps}`,
);
ok(
  !(await get(`/api/session?user=${U}`)).blocks.some(
    (b: any) => b.kind === "grammar-review",
  ),
...
const logged = (
  db2
    .prepare(
      "SELECT COUNT(*) n FROM attempt WHERE user_id = ? AND kind = 'review'",
    )
    .get(U) as { n: number }
).n;
db2.close();
...
const first = (await get(`/api/session?user=${U}`)).blocks.find(
  (b: any) => b.kind === "grammar-review",
)?.payload.cards[0].drills[0].q;
setDue(c!.id, "due = datetime('now','-1 day'), reps = 1");
const second = (await get(`/api/session?user=${U}`)).blocks.find(
  (b: any) => b.kind === "grammar-review",
)?.payload.cards[0].drills[0].q;
ok(
  first !== second,
  "a different drill on the next visit",
