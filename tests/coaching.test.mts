...
eq(
  coachingBrief({ mistakes: ["  "], stuck: [""] }),
  null,
  "blank strings do not count as memory",
);
...
ok(
  b.includes("Nominative article where accusative is needed"),
  "the first mistake is named",
);
ok(b.includes("Verb not in second position"), "and the second");
ok(
  b.includes("die Rechnung") && b.includes("vielleicht"),
  "so are the stuck words",
);
...
ok(
  /do not name a\s+grammar rule/i.test(b),
  "told not to name the rule",
  b.split("\n").at(-2),
);
...
const onlyMistakes = coachingBrief({
  mistakes: ["Wrong plural form"],
  stuck: [],
})!;
ok(
  onlyMistakes.includes("Wrong plural form"),
  "mistakes alone still produce a brief",
);
ok(
  !/slipping away/i.test(onlyMistakes),
  "with no sentence about words that are not there",
);
