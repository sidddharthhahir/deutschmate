...
eq(
  scoped("dm.outbox.v1", "sid"),
  "dm.outbox.v1:sid",
  "the shape of a scoped key",
);
ok(
  scoped("dm.outbox.v1", "sid") !== scoped("dm.outbox.v1", "mira"),
  "two learners, two keys",
);
eq(
  scoped("dm.outbox.v1", "MIRA"),
  "dm.outbox.v1:mira",
  "case does not fork the key",
);
eq(
  scoped("dm.outbox.v1", ""),
  `dm.outbox.v1:${DEFAULT_USER}`,
  "a blank name falls back",
);
...
eq(
  userFromCookie("theme=dark"),
  DEFAULT_USER,
  "absent means the default learner",
);
eq(userFromCookie(""), DEFAULT_USER, "no cookies at all");
eq(
  userFromCookie("dm_uid=%20Anna-Lena%20"),
  "anna-lena",
  "url-encoded and normalised",
);
eq(
  userFromCookie("dm_uidx=zzz"),
  DEFAULT_USER,
  "a lookalike name is not the cookie",
);

section("it reads the cookie sign-in actually sets");
/*
 * This read dm_user, which sign-in stopped writing when it moved to dm_uid — so every learner fell
 * back to DEFAULT_USER and shared one set of localStorage buckets: cached plan, resume offer, tour
 * flag, and the queue of answers given offline.
 */
eq(
  userFromCookie("dm_uid=u_9f2k"),
  "u_9f2k",
  "an opaque account id, which is what sign-in sets",
);
eq(
  userFromCookie("dm_uid=mira; dm_user=sid"),
  "mira",
  "dm_uid wins over the pre-sign-in cookie",
);
eq(
...
eq(
  userFromCookie("dm_uid=; dm_user=mira"),
  "mira",
  "an empty dm_uid is not an identity",
);

section("no document means the server, and the server never guesses wrong");
ok(
  typeof (globalThis as any).document === "undefined",
  "no document in this process",
);
