/**
 * Does config.ts actually control anything? Moving them into one place only helps if the call
 * sites were rewired too — and five of them were not.
 * needs: nothing
 */
...
for (const name of names) {
  ...
  ok(
    ...
    users.length
      ? users[0].f.replace("src/", "")
      : "NOTHING READS IT — the value is decorative",
  );
}
...
ok(
  PACE_CUT_ACCURACY > 0 && PACE_CUT_ACCURACY <= 1,
