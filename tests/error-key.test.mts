...
eq(
  patternFor("Du gehst nach Hause", "Du gehe nach Hause"),
  "v:-e→-st",
  "a verb ending",
);
eq(
...
eq(
  patternFor("Ich gehe heute ins Kino", "Ich gehe heute ins Kino"),
  null,
  "no mistake, no key",
);
...
eq(
  patternFor(
    ...
    "Ich will Zug Köln morgen",
  ),
  null,
  "a rewrite with four differences has no single lesson",
);
eq(
  patternFor("Ich bin müde", "Ich bin Müde"),
  "case",
  "only the capital changed",
);
eq(
  patternFor("Ich möchte schön wohnen", "Ich möchte schon wohnen"),
  "sp:umlaut",
  "only an umlaut",
);
eq(
...
const REAL: [expected: string, got: string, about: string][] = [
  ...
  [
    ...
    "Ich wohne bei mein Bruder",
    "possessive in the dative",
  ],
  ...
  [
    ...
    "nicht once there is a possessive",
  ],
  [
  ...
  [
    ...
    "Ich wohne vor zwei Jahren hier",
    "seit vs vor",
  ],
  [
  ...
  [
    ...
    "Ich liege das Buch auf den Tisch",
    "legen vs liegen",
  ],
...
const row = db.prepare(
  "SELECT explain_md, source, tag FROM error_pattern WHERE signature = ?",
);
const total = db
  .prepare("SELECT COUNT(*) AS n FROM error_pattern WHERE source = 'prebuilt'")
  .get() as { n: number } | undefined;

section("the table is actually populated");
ok(
  (total?.n ?? 0) >= 200,
  "at least 200 prebuilt patterns are seeded",
  total?.n ?? 0,
);
...
for (const [expected, got, about] of REAL) {
  ...
  const hit = key
    ? (row.get(key) as { explain_md: string } | undefined)
    : undefined;
  if (hit) specific++;
...
section(
  "every tag has a last-resort row, so nothing falls through to a bare label",
);
const TAGS = [
  "article-gender",
  "article-akkusativ",
...
const all = db
  .prepare(
    "SELECT signature, explain_md FROM error_pattern WHERE source = 'prebuilt'",
  )
  .all() as { signature: string; explain_md: string }[];
const leaky = all.filter(
  (r) =>
    /\bthis (noun|word|verb) (is|takes) (masculine|feminine|neuter)\b/i.test(
      r.explain_md,
    ) && !/^w:/.test(r.signature),
);
...
const avg = Math.round(
  all.reduce((n, r) => n + r.explain_md.length, 0) / all.length,
);
...
const SLUGS = new Set(
  (db.prepare("SELECT slug FROM grammar").all() as { slug: string }[]).map(
    (r) => r.slug,
  ),
);
