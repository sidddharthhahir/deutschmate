...
const units = db
  .prepare(
    "SELECT id, ord, word_ids_json FROM unit WHERE level = 'A1.1' ORDER BY ord",
  )
  .all() as { id: string; ord: number; word_ids_json: string }[];

const target = units.find(
  (u) => (JSON.parse(u.word_ids_json) as string[]).length > 12,
);
if (!target) {
...
ok(
  s1.unit.id === target.id,
  "landed on the oversized unit",
  `unit ${s1.unit.ord}`,
);
const nv = s1.blocks.find((b: any) => b.kind === "new-vocab");
ok(
  nv?.payload.words.length === 12,
  "twelve words offered today",
  nv?.payload.words.length,
);
...
const done1 = await post("/api/session", {
  ...
  minutes: 30,
  blocks: ["new-vocab"],
  completeUnit: target.id,
});
ok(
  done1.unitDone === false,
  "the unit is NOT marked complete yet",
  `wordsLeft=${done1.wordsLeft}`,
);
ok(
  done1.wordsLeft === size - 12,
  "the remainder is reported honestly",
  `${done1.wordsLeft} of ${size}`,
);
...
ok(
  nv2?.payload.words.length === size - 12,
  "and only the leftovers are offered",
...
ok(
  done2.unitDone === true,
  "now the unit completes",
  `wordsLeft=${done2.wordsLeft}`,
);
...
ok(
  introduced === size,
  "every word in the unit was actually introduced",
  `${introduced} of ${size}`,
);
