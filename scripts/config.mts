/**
 * What is this install configured to do? npm run config Every `process.env` read in the app used
 * to have its own fallback, and every one failed silently: `DEUTSCHMATE_BUDGT=5` is not an error,
 * it is a budget of $5 because the misspelled name was never read.
 */
import "./load-env.mts";
...
for (const row of describe()) {
  console.log(
    `  ${row.name.padEnd(24)} ${row.value.padEnd(28)} ${dim(row.note)}`,
  );
}
