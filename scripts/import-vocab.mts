/**
 * Grow the deck from A1–thin-B1 to a full B1, using openly-licensed sources. node
 * scripts/import-vocab.mts # download, pick, emit node scripts/import-vocab.mts --dry # show what
 * it would add node scripts/import-vocab.mts --target 2400 WHY NOT THE GOETHE WORDLIST.
 */
...
/** Parts of speech worth teaching. `pron` and `article` are deliberately absent. */
...
const NOT_A_MEANING =
  /^(only )?used\b|^(expressing|indicating|denoting)\b|^see\b|^alternative\b/i;
...
async function ensure(file: string, url: string, label: string) {
  if (existsSync(file)) {
    console.log(
      `  cached  ${label}  ${(statSync(file).size / 1048576).toFixed(1)} MB`,
    );
    return;
  }
  ...
  console.log(
    `  saved   ${label}  ${(statSync(file).size / 1048576).toFixed(1)} MB`,
  );
}
...
if (existsSync(OUT_WORDS)) {
  for (const w of JSON.parse(readFileSync(OUT_WORDS, "utf8")) as {
    id: string;
  }[])
    mine.add(w.id);
}
if (mine.size)
  console.log(`  ignoring ${mine.size} words from this importer's last run`);

const existing = new Set<string>();
const existingCount: Record<string, number> = {};
for (const w of db
  .prepare("SELECT id, lemma, level, plural, forms_json FROM word")
  .all() as {
  id: string;
  lemma: string;
  level: string;
...
for (const lv of LEVELS) {
  ...
  console.log(
    `  ${lv}  have ${String(have).padStart(4)}  want ${String(want).padStart(4)}  add ${need[lv]}`,
  );
}
const totalNeeded = LEVELS.reduce((n, lv) => n + need[lv], 0);
console.log(
  `  total in deck ${running} → target ${TARGET}, adding up to ${totalNeeded}`,
);
...
function genderOf(e: Record<string, unknown>): string | null {
  const ht = (
    e.head_templates as { args?: Record<string, string> }[] | undefined
  )?.[0];
  const a1 = ht?.args?.["1"];
...
function pluralOf(e: Record<string, unknown>): string | null {
  const forms =
    (e.forms as { form?: string; tags?: string[] }[] | undefined) ?? [];
  const p = forms.find(
...
function glossOf(e: Record<string, unknown>): string | null {
  ...
  for (const s of senses) {
    ...
    if (s.tags?.some((t) => WEAK_SENSE_TAGS.has(t) || t === "form-of"))
      continue;
...
const rl = createInterface({
  input: createReadStream(KAIKKI, "utf8"),
  crlfDelay: Infinity,
});
let scanned = 0;
for await (const line of rl) {
  ...
  if (e.pos === "verb") {
    for (const f of (e.forms ?? []) as { form?: string; tags?: string[] }[]) {
      if (f.form && f.tags?.includes("participle"))
        participles.add(lower(f.form));
    }
  }
...
rl.close();
console.log(
  `Scanned ${scanned.toLocaleString("en")} entries, matched ${found.size}`,
);
...
function isDerived(e: Entry): boolean {
  ...
  if (w.endsWith("er")) {
    ...
    if (
      existing.has(stem) ||
      existing.has(stem + "e") ||
      existing.has(unUmlaut)
    )
      return true;
  }

  /*
   * geführt, gebaut, geschlossen, verändert, vorgestellt — all participles that Wiktionary also
   * files as adjectives.
   */
...
/** A German infinitive always ends in -n. */
...
for (const w of freq) {
  ...
  /* No gender means no card worth having: the whole point of a German noun card is der/die/das. */
...
for (const lv of LEVELS) {
  ...
  for (const u of lvUnits) {
    const base = (JSON.parse(u.word_ids_json) as string[]).filter(
      (id) => !mine.has(id),
    );
  ...
  for (const w of lvWords) {
    ...
    for (const u of lvUnits)
      if (finalSize.get(u.id)! < finalSize.get(target.id)!) target = u;
...
console.log(
  `Then:  npm run audio   (fetches pronunciations for the new words)`,
);
