...
export default function PendingTexts({ initial }: { initial: number }) {
  ...
  const apply = useCallback(
    (data: { pending: number; resolved: Resolved[] }) => {
  ...
  return (
    <div className="border-line-sub bg-raised mt-4 rounded-xl border p-4">
      {pending > 0 && (
        <>
          <p className="text-accent/90 text-[14px]">
            {pending} {pending === 1 ? "Text wartet" : "Texte warten"} auf
            Korrektur
          </p>
      ...
      {got.map((r) => (
        <div
          key={r.id}
          className="border-line-sub mt-4 border-t pt-4 first:border-0 first:pt-0"
        >
          ...
          {!r.corrections.length && (
            <p className="font-serif text-accent mt-2 text-[15px]">
