...
export default async function ProgressPage() {
  ...
  const stuck = leeches(user.id, LEECH_THRESHOLD, 8).filter(
    (l) => l.suspended === 0,
  );
    ...
    >
      <>
        ...
        >
          ...
          <Section title={`Grammatik — ${gram.solid} von ${gram.total}`}>
            ...
            <p className="text-muted mt-3 max-w-[62ch] text-[12.5px] leading-relaxed">
              {plural(gram.inDeck, "Regel", "Regeln")} {is(gram.inDeck)}{" "}
              eingeführt und {is(gram.inDeck) === "ist" ? "wird" : "werden"}{" "}
              wiederholt, {gram.solid} davon
              {gram.solid === 1 ? " sitzt" : " sitzen"} (3+ Wiederholungen).
              Grammatik läuft auf derselben Vergessenskurve wie Wörter — eine
              Regel, die du einmal gesehen hast, ist keine Regel, die du kannst.
            </p>
          </Section>
        </Group>
        ...
        >
          {pace && (
            <Section title="Tempo">
              <p className="font-serif text-[19px] leading-relaxed">
                {plural(pace.done, "Unit", "Units")} in{" "}
                {plural(pace.days, "Tag", "Tagen")} — das sind{" "}
                <span className="text-accent">{pace.perWeek} pro Woche</span>.
                Bei diesem Tempo sind die restlichen {pace.remaining} in etwa{" "}
                {plural(pace.weeksLeft, "Woche", "Wochen")} durch, also um den{" "}
              ...
              <p className="text-muted mt-3 max-w-[62ch] text-[12.5px] leading-relaxed">
                Das ist Rechnen mit deinem bisherigen Tempo, keine Aussage über
                dein Deutsch. Es sagt, wann dir der Kurs ausgeht — nicht, dass
                du dann B1.2 sprichst.
              </p>
            </Section>
          )}
          ...
          <Section
            title={`Units — ${completed.size} von ${unitsByLevel.length}`}
          >
        ...
        >
          ...
          {tags.length > 0 && (
            <Section title="Häufigste Fehler · 30 Tage">
              ...
              <div className="space-y-1">
                {tags.map((t) => (
                  ...
                  >
                    ...
                    <span className="font-mono text-muted flex-none">
                      {t.n}× →
                    </span>
                  </Link>
                ))}
              ...
              <p className="text-muted mt-3 text-[12.5px]">
                Die obersten drei landen morgen automatisch im Fix-Block.
                Antippen zeigt jedes einzelne Beispiel.
              </p>
            </Section>
          )}
          ...
          {stuck.length > 0 && (
            <Section title={`Problemwörter · ${stuck.length}`}>
              <p className="text-muted mb-4 max-w-[62ch] text-[12.5px] leading-relaxed">
                Mindestens {LEECH_THRESHOLD}-mal wieder vergessen. Häufiger
                wiederholen hilft hier nicht mehr — das passiert ja bereits.
              </p>
                ...
                {stuck.map((l) => (
                  ...
                  >
                    <span className="font-serif text-[17px]">
                      <Noun article={l.pos === "noun" ? l.article : null}>
                        {l.lemma}
                      </Noun>
          ...
          {exams.length > 0 && (
            <Section title={`Übungstests · ${exams.length}`}>
                    ...
                    >
                      ...
                      <span className="font-mono text-secondary text-[12px] tabular-nums">
                        {sections
                          .map(
                            (s) =>
                              `${s.title.slice(0, 4)} ${s.correct}/${s.total}`,
                          )
                          .join(" · ")}
              ...
              <p className="text-muted mt-3 max-w-[62ch] text-[12.5px] leading-relaxed">
                Aus dem Inhalt dieser App gebaut — kein offizieller Modellsatz
                und keine Aussage darüber, ob du die echte Prüfung bestehen
                würdest.
              </p>
            </Section>
          )}
          ...
          {sounds.length > 0 && (
            <Section title="Aussprache">
              ...
              <p className="text-muted mt-3 max-w-[62ch] text-[12.5px] leading-relaxed">
                Gezählt aus echten Erkennungsergebnissen — kein
                Aussprache-Score.
              </p>
            </Section>
          )}
          ...
          <Section title="Kosten · 30 Tage">
            {spend.calls === 0 ? (
              <p className="text-muted max-w-[62ch] text-[14px] leading-relaxed">
                Noch kein einziger Modellaufruf. Alles, was du bisher gemacht
                hast, lief lokal — Wiederholungen, Lücken, Tests und Wortschatz
                kosten nichts.
                ...
                {hasKey ? (
                  ...
                  <>
                    {" "}
                    Für Gespräch, Schreibkorrektur und neue Erklärungen brauchst
                    du einen eigenen Anthropic-Schlüssel —{" "}
                    <Link
                      href="/einstellungen"
              ...
              <>
                <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
                  ...
                  <Stat
                    n={spend.cacheShare}
                    label="% aus Cache"
                    hint="kostet 10% des Preises"
                  />
                  <Stat
                    n={Math.round(
                      (spend.input + spend.output + spend.cacheRead) / 1000,
                    )}
                    label="k Tokens"
                ...
                <div className="mt-6">
                  <div className="flex items-baseline justify-between text-[12.5px]">
                    <span className="text-secondary">
                      Budget · {budget.spent.toFixed(2)} $ von{" "}
                      {budget.ceiling.toFixed(2)} $
                    </span>
                ...
                <p className="text-muted mt-4 max-w-[62ch] text-[12.5px] leading-relaxed">
                  ...
                  {unpriced.length > 0 && (
                    <span className="text-das">
                      {" "}
                      {plural(
                        unpriced.reduce((n, m) => n + m.calls, 0),
                        "Aufruf",
                        "Aufrufe",
                      )}{" "}
                      mit {unpriced.map((m) => m.model).join(", ")}{" "}
                      {unpriced.length === 1 ? "ist" : "sind"} hier nicht
                      eingerechnet — dieses Modell steht nicht in der
                      Preisliste.
                    </span>
                  )}
                  {budget.remaining <= 0 ? (
                    <span className="text-das">
                      {" "}
                      Das Budget ist aufgebraucht: Gespräch, Schreibkorrektur
                      und neue Erklärungen pausieren bis zum nächsten Fenster.
              ...
              <div className="flex h-[60px] items-end gap-1">
                {days.map((d) => (
                  <div
                    ...
                    style={{
                      height: `${Math.min(100, (d.minutes / 90) * 100)}%`,
                    }}
                  />
                ))}
              </div>
